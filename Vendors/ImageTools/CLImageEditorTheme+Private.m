...
static NSBundle* getBundle() {
    return [NSBundle bundleWithPath:@"/Library/Application Support/WAEnhancer/"];
}

- (NSBundle*)bundle
{
    return [NSBundle bundleWithPath:[getBundle() pathForResource:self.bundleName ofType:@"bundle"]];
}
...
+ (UIImage*)imageNamed:(Class)path image:(NSString*)image
{
    ...
    return [UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"/Library/Application Support/WAEnhancer/%@.bundle/%@/%@/%@", self.bundleName, path, theme.toolIconColor, image]];
}
