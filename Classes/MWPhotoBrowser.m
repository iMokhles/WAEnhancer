...
//        
//    }];
//    [requestAct activityDidFinish:YES];
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    requestAct = [[WALPActivity alloc] init];
                    // editorActivity = [[CLImageActivity alloc] init];
                    NSArray *appActivity = @[requestAct/*, editorActivity*/];
                    self.activityViewController = [[UIActivityViewController alloc] initWithActivityItems:items applicationActivities:appActivity];
                    ...
                    [self.activityViewController setCompletionHandler:^(NSString *activityType, BOOL completed) {
                        if([activityType isKindOfClass:[WALPActivity class]]){
                            NSLog(@"WallpaperPlus**************");
